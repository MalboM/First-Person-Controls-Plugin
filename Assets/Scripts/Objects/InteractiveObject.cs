...
public class InteractiveObject : MonoBehaviour {
    ...
    void Drop(PlayerActionController ac)
    {
        ...
        if (canBeHeld) 
        {
            ...
            if (!Physics.Raycast(this.transform.position, ac.transform.forward, out hitInfo, castDistance, mask))
            {
                ...
                this.GetComponent<Rigidbody>().AddForce(ac.transform.forward * DROP_IMPULSE, ForceMode.VelocityChange);
            }
            else
            {
    ...
    {
        Drop(ac);
        this.GetComponent<Rigidbody>().AddForce(direction*throwStrength,ForceMode.Impulse);
    }
    #endregion
